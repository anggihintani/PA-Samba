...
const handleFieldOperator = (context, condition, conditionName, fieldMapping, prev) => {
    ...
    if ((utils_1.isMatchFilterMapping(fieldMapping) ||
        utils_1.isCustomFilterMapping(fieldMapping)) &&
    ...
    if (conditionName in prev) {
        // eslint-disable-next-line
        condition = Object.assign({}, condition, prev[conditionName]);
    }
    return {
        condition,
        conditionName,
...
function buildConditionsObject(context, conditions, mapping) {
    return Object.keys(conditions).reduce((prev, currentKey) => {
        ...
        if (fieldMapping &&
            !utils_1.isCustomFilterMapping(fieldMapping) &&
            !utils_1.isMatchFilterMapping(fieldMapping)) {
            return prev;
        }
        if (conditionName === 'AND') {
            ...
            if (utils_1.isCustomFilterMapping(fieldMapping)) {
                if (fieldMapping.format && typeof fieldMapping.format === 'function') {
                    ...
                    return Object.assign({}, prev, customCondition);
                }
            }
            ({ condition, conditionName } = handleFieldOperator(context, condition, conditionName, fieldMapping, prev));
        }
        conditionName =
            utils_1.isMatchFilterMapping(fieldMapping) && fieldMapping.key
                ? fieldMapping.key
                : conditionName;
        return Object.assign({}, prev, { [conditionName]: condition });
    }, {});
}
function buildMongoConditionsFromFilters(context, filters = {}, mapping = {}) {
    ...
    const filtersKeysGrouped = Object.keys(filters).reduce((prev, key) => {
        ...
        return Object.assign({}, prev, { [type]: Object.assign({}, prev[type], { [key]: filters[key] }) });
    }, {
    ...
    const conditions = buildConditionsObject(context, Object.assign({}, filtersKeysGrouped.MATCH_1_TO_1, filtersKeysGrouped.CUSTOM_CONDITION), mapping);
